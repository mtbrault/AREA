...
class ReactionScreen extends Component {
	...
	getData() {
		...
		axios.post(`${Ip}/service/${this.service}/get/reactions`, {
			actionId: this.id
		}).then((res) => {
			if (res) {
	...
	_renderTypeItem = (type, i) => <ReactionCard content={"type"} data={type} idx={i} update={this.addAction.bind(this)} id={this.id}/>
	_renderElemItem = (elem, i) => <ReactionCard content={"elem"} data={elem} idx={i} update={this.removeAction.bind(this)} id={this.id}/>
	...
	render() {
      ...
      />
			<View style={{width: '100%', height: 30, backgroundColor: 'white'}}>
					<Text style={{color: 'black', fontSize: 15, fontWeight: 'bold'}}>Reactions :</Text>
				</View>
			  <Grid
        style={styles.list}
