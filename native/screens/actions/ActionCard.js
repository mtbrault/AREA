...
class ActionScreen extends Component {
	...
	sendAddRequest(param = null) {
		axios.post(`${Ip}/service/${this.props.service}/add/actions`, {
	...
	deleteAction() {
		axios.post(`${Ip}/service/${this.props.service}/delete/actions`, {
	...
	_renderElemItem = (data, i) => (
		...
		>
			...
			justifyContent: 'space-between', }]}
			>
				<TouchableHighlight onPress={() => this.props.navigation.navigate('Reactions', {id: this.props.data.id, service: this.props.service})} underlayColor="white">
					<Text style={styles.cardText}>{this.props.data.count}</Text>
				</TouchableHighlight>
